-- =====================================================
-- RENAME CONTENT LIBRARY TABLES (FIXED)
-- Run this to rename all tables with content_library_ prefix
-- =====================================================

-- =====================================================
-- 1. DROP ONLY CONTENT LIBRARY SPECIFIC TRIGGERS
-- =====================================================
...
-- =====================================================
-- 2. DROP ONLY CONTENT LIBRARY SPECIFIC FUNCTIONS
-- =====================================================
...
-- DON'T drop update_updated_at_column() - other tables use it
...
create index if not exists idx_content_library_assets_status on content_library_assets(status) where deleted_at is null;
create index if not exists idx_content_library_assets_uploaded_at on content_library_assets(uploaded_at desc) where deleted_at is null;
create index if not exists idx_content_library_assets_uploaded_by on content_library_assets(uploaded_by) where deleted_at is null;
create index if not exists idx_content_library_assets_file_type on content_library_assets(file_type) where deleted_at is null;
create index if not exists idx_content_library_assets_campaign_id on content_library_assets(campaign_id) where deleted_at is null;
create index if not exists idx_content_library_assets_product_id on content_library_assets(product_id) where deleted_at is null;
create index if not exists idx_content_library_assets_tags on content_library_assets using gin(tags);
create index if not exists idx_content_library_assets_metadata on content_library_assets using gin(metadata);
create index if not exists idx_content_library_assets_deleted_at on content_library_assets(deleted_at);

create index if not exists idx_content_library_comments_asset on content_library_comments(asset_id, created_at desc) where deleted_at is null;
create index if not exists idx_content_library_comments_parent on content_library_comments(parent_id) where deleted_at is null;
create index if not exists idx_content_library_comments_author on content_library_comments(author_id) where deleted_at is null;
create index if not exists idx_content_library_comments_deleted_at on content_library_comments(deleted_at);

create index if not exists idx_content_library_activity_asset on content_library_activity(asset_id, created_at desc);
create index if not exists idx_content_library_activity_actor on content_library_activity(actor_id);
create index if not exists idx_content_library_activity_created_at on content_library_activity(created_at desc);

create index if not exists idx_content_library_versions_asset on content_library_versions(asset_id, version_number desc);
create index if not exists idx_content_library_versions_created_by on content_library_versions(created_by);

create index if not exists idx_content_library_user_profiles_role on content_library_user_profiles(role);
create index if not exists idx_content_library_campaigns_status on content_library_campaigns(status);
create index if not exists idx_content_library_campaigns_created_by on content_library_campaigns(created_by);
create index if not exists idx_content_library_products_category on content_library_products(category);
create index if not exists idx_content_library_products_created_by on content_library_products(created_by);

-- =====================================================
-- 6. CREATE CONTENT LIBRARY SPECIFIC FUNCTIONS
-- =====================================================

-- Function to log content library activity (new function name)
...
-- Function to auto-increment version numbers (new function name)
...
begin
  ...
  raise notice '💡 Note: Kept existing update_updated_at_column() function (used by other tables)';
end $$;