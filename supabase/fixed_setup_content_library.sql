-- =====================================================
-- FIXED SUPABASE SETUP FOR CONTENT LIBRARY
...
-- FIXED Function to log asset activity
...
drop policy if exists "Editors and admins can manage campaigns" on campaigns;

drop policy if exists "Everyone can view products" on products;
drop policy if exists "Editors and admins can manage products" on products;

drop policy if exists "Everyone can view non-deleted assets" on assets;
drop policy if exists "Authenticated users can create assets" on assets;
drop policy if exists "Asset owners and editors can update assets" on assets;
drop policy if exists "Asset owners and admins can soft delete assets" on assets;

drop policy if exists "Everyone can view non-deleted comments" on asset_comments;
drop policy if exists "Authenticated users can create comments" on asset_comments;
drop policy if exists "Comment authors can update own comments" on asset_comments;
drop policy if exists "Comment authors and admins can soft delete comments" on asset_comments;

drop policy if exists "Everyone can view activity logs" on asset_activity;
drop policy if exists "System can insert activity logs" on asset_activity;

drop policy if exists "Everyone can view asset versions" on asset_versions;
drop policy if exists "Authenticated users can create versions" on asset_versions;
...
-- CAMPAIGNS POLICIES (simplified - everyone can view and manage for now)
...
create policy "Authenticated users can update comments" on asset_comments
  for update using (auth.uid() is not null);

create policy "Authenticated users can delete comments" on asset_comments
  for delete using (auth.uid() is not null);
...
drop policy if exists "Anyone can view content library files" on storage.objects;
...
do $$
begin
  raise notice '✅ Content Library Database Setup Complete!';
  raise notice '📦 Tables Created: user_profiles, campaigns, products, assets, asset_comments, asset_activity, asset_versions';
  raise notice '🪣 Storage Bucket: content-library';
  raise notice '🔒 RLS Policies: Applied';
  raise notice '⚡ Functions: get_assets_with_stats(), get_asset_comments_with_replies()';
end $$;