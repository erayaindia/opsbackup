-- Extend Existing Products Table for Lifecycle Management
-- Created: 2025-09-13
-- Description: Extends existing products table to support both inventory and lifecycle management

-- Enable UUID extension (if not already enabled)
CREATE EXTENSION IF NOT EXISTS "uuid-ossp";

-- Create custom types/enums for lifecycle management
CREATE TYPE product_priority AS ENUM ('low', 'medium', 'high');
CREATE TYPE product_stage AS ENUM ('idea', 'production', 'content', 'scaling', 'inventory'); -- Added 'inventory' for existing products
...
ALTER TABLE products ADD COLUMN IF NOT EXISTS stage product_stage DEFAULT 'inventory'; -- Default to 'inventory' for existing products
...
ALTER TABLE products ADD COLUMN IF NOT EXISTS potential_score INTEGER DEFAULT 0 CHECK (potential_score >= 0 AND potential_score <= 100);
...
-- 1. Product ideas table (stage = 'idea')
...
-- 2. Product production table (stage = 'production')
...
-- 3. Product content table (stage = 'content')
...
-- 4. Product scaling table (stage = 'scaling')
...
-- 5. Product categories (Many-to-Many) - Enhanced version of existing category relationship
...
WHERE NOT EXISTS (
    ...
    WHERE pc.product_id = p.id AND pc.category = c.name
)
...
-- 7. Product suppliers (Many-to-Many with enhanced data)
...
-- 12. Product deliverables (for content stage)
...
-- 13. Product experiments (for scaling stage)
...
-- 14. Product asset links (for content stage)
...
-- New indexes for lifecycle management columns on products table
...
-- Enable RLS on new tables (products already has RLS from inventory system)
...
-- View to get products with all their related data (both inventory and lifecycle)
...
SELECT
    ...
    ARRAY_AGG(DISTINCT pc.category) FILTER (WHERE pc.category IS NOT NULL) as lifecycle_categories,
    ARRAY_AGG(DISTINCT pt.tag) FILTER (WHERE pt.tag IS NOT NULL) as tags,
    -- Include original inventory category for backward compatibility
    cat.name as inventory_category
FROM products p
...
GROUP BY p.id, pi.notes, pi.problem_statement, pi.opportunity_statement,
         ...
         pi.estimated_selling_price, creator.full_name, assignee.full_name, cat.name;

-- View for lifecycle products only (excluding inventory products)
CREATE OR REPLACE VIEW lifecycle_products AS
SELECT * FROM products_with_details
WHERE stage != 'inventory' OR stage IS NULL;

-- View for inventory products only (excluding lifecycle products)
...
-- Function to convert inventory product to lifecycle product
CREATE OR REPLACE FUNCTION convert_to_lifecycle_product(
    product_id UUID,
    new_stage product_stage DEFAULT 'idea',
    new_priority product_priority DEFAULT 'medium'
) RETURNS BOOLEAN AS $$
BEGIN
    UPDATE products
    SET
        ...
        internal_code = COALESCE(internal_code, generate_internal_code()),
        working_title = COALESCE(working_title, name)
    WHERE id = product_id;
...
BEGIN
    RAISE NOTICE '===============================================';
    RAISE NOTICE 'Product Lifecycle Extension Migration Completed Successfully!';
    RAISE NOTICE '===============================================';
    RAISE NOTICE 'Extended existing products table with lifecycle management';
    RAISE NOTICE 'Created % new supporting tables', 14;
    RAISE NOTICE 'Created % storage buckets', 3;
    RAISE NOTICE 'Migrated existing category relationships';
    RAISE NOTICE 'All existing inventory products preserved with stage = "inventory"';
    RAISE NOTICE 'New lifecycle products will use stage = "idea", "production", "content", "scaling"';
