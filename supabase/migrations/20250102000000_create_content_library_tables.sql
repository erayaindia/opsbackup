...
-- Complete setup for assets, timestamped comments, and activity logging with proper RLS
...
-- Users profile table (optional - for storing additional user info)
...
-- Campaigns table (optional - for organizing assets)
...
-- Products table (optional - for categorizing assets)
...
-- Asset comments table (timestamped + threaded)
create table if not exists asset_comments (
  id uuid primary key default gen_random_uuid(),
  asset_id uuid not null references assets(id) on delete cascade,
  author_id uuid references auth.users(id) on delete set null,
  parent_id uuid null references asset_comments(id) on delete cascade,
  timestamp_sec int null, -- null for images/docs, video timestamp for videos
...
-- Asset activity log table (audit trail)
...
-- Asset versions table (for tracking changes)
...
create policy "Asset owners and admins can soft delete assets" on assets
  for update using (
    ...
    -- Only allow updating deleted_at field for soft delete
...
create policy "System can insert activity logs" on asset_activity
  for insert with check (true); -- Controlled by trigger function
...
-- Function to get asset with comment count
...
-- Insert default admin user profile (update the UUID with actual user ID)
-- insert into user_profiles (id, full_name, role) 
-- values ('your-admin-user-uuid', 'Admin User', 'admin');

-- Insert sample campaign
-- insert into campaigns (name, description, status) 
-- values ('Sample Campaign', 'Demo campaign for testing', 'active');

-- Insert sample product
-- insert into products (name, description, category) 
-- values ('Sample Product', 'Demo product for testing', 'Electronics');

-- =====================================================
-- 11. GRANTS (IF NEEDED)
-- =====================================================

-- Grant usage on sequences and functions to authenticated users
