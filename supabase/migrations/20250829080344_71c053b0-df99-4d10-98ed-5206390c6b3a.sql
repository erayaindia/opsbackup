-- Fix profiles table to work with Supabase Auth
-- Add user_id column for Supabase auth users
...
USING (EXISTS (
  ...
  WHERE cm1.user_id = auth.uid()::text AND cm2.user_id = profiles.clerk_user_id
));

-- Add columns to other tables for Supabase auth compatibility
