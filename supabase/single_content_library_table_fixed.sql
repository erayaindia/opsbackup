-- =====================================================
-- SINGLE CONTENT LIBRARY TABLE SETUP (FIXED)
...
-- Drop existing single table if it exists
...
create table content_library (
  ...
  -- Structure: [{id, actor_id, actor_name, action, timestamp}]
...
begin
  ...
  if tg_op = 'INSERT' then
    ...
    if old.status::text != new.status::text then
      action_text := 'Status changed from ' || old.status::text || ' to ' || new.status::text;
...
-- Allow public access to content-library bucket files
create policy "Content library files are publicly accessible" on storage.objects
  for select using (bucket_id = 'content-library');

-- Allow authenticated users to upload to content-library bucket
create policy "Authenticated users can upload content library files" on storage.objects
  for insert with check (
...
create policy "Users can update own content library files" on storage.objects
  for update using (
    bucket_id = 'content-library' and
...
create policy "Users can delete own content library files" on storage.objects
  for delete using (
    bucket_id = 'content-library' and
    auth.uid()::text = (storage.foldername(name))[1]
  );
